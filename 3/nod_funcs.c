...
void addType(node* root, char* table_name, char* my_opp)
{
    ...
    if (root->son != NULL)
    {
        ...
        else if(strcmp(sons->str_type,"CharLit") == 0)
        {
            sons->anote_type = (char*)malloc(sizeof(char)*strlen("char")+1);
            strcpy(sons->anote_type,"int");
        }
    ...
    if (strcmp(my_opp, "Add") == 0)
    {
        ...
        else if ( (n_pointers_one == n_pointers_two) && (strcmp(type_son_one, type_son_two) == 0))
        {
...
void getType(node *root, char* table_name, char* symbol_name)
{
    ...
    else if(strcmp(root->str_type,"CharLit") == 0)
    {
        root->anote_type = (char*)malloc(sizeof(char)*strlen("char")+1);
        strcpy(root->anote_type,"int");
        return;
    }
    ...
    else if (strcmp(root->str_type, "Add") == 0 || (strcmp(root->str_type, "Sub") == 0) || (strcmp(root->str_type, "Mul") == 0) || (strcmp(root->str_type, "Div") == 0) || (strcmp(root->str_type, "Mod") == 0))
    {
    ...
    else if (strcmp(root->str_type, "Comma") == 0)
    {
        if (root->son != NULL)
        {
            ...
            //getType(root, func_name, last_value);
            if (and_this->value != NULL)
            {
                last_value = (char*)calloc(strlen(and_this->value)+1, sizeof(char));
                strcpy(last_value, and_this->value);
